...
set chunk_size_choices=50 256 512 768 1024
...
call :display_option 8  docs_dir "           " !docs_dir! "  "       "[docs/security, docs/airrag_docs, docs/email, docs/geography, docs/reports, docs/wrk, NONE]"
...
REM Get user input
set /p choice=Choose option, R to run, Q to set question, D to set docs_dir, or X to exit: 

REM Rotate option, run script, or quit based on input
if /i "%choice%"=="x" goto end
if /i "%choice%"=="r" goto run_script
if /i "%choice%"=="q" goto get_question
if /i "%choice%"=="d" goto set_docs_dir
...
:set_docs_dir
set /p new_docs_dir=Enter the new docs_dir path: 
if "%new_docs_dir%"=="" (
    echo No input provided. docs_dir remains as !docs_dir!
) else (
    set docs_dir=%new_docs_dir%
...
REM 1 retriever_name:    <giraffe> ... [Naive_ST_FAISS_Retriever, giraffe, ostrich]
